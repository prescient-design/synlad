"""Adapted from https://github.com/facebookresearch/all-atom-diffusion-transformer/blob/main/src/models/encoders/transformer.py
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.ph4_type_embedder = nn.Embedding(max_num_elements, d_model)
    ...
    def forward(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        """
        Transformer encoder forward pass without positional embedding.
        Args:
            batch: Data object with the following attributes:
                ph4_channels (torch.Tensor): Pharmacophore types in the batch
                ph4_coords (torch.Tensor): Cartesian coordinates of pharmacophores in the batch
                num_ph4_channels (torch.Tensor): Number of pharmacophore channels in the batch
                batch (torch.Tensor): Batch index for each pharmacophore
                token_idx (torch.Tensor): Token index for each pharmacophore
        """
        x = self.ph4_type_embedder(batch.ph4_channels)  # (n, d)
        x += self.pos_embedder(batch.ph4_coords)
        ...
        return {
            "x": x,
            "num_ph4_channels": batch.num_ph4_channels,
            "batch": batch.batch,
